...
const customCrit5e = (() => { // eslint-disable-line no-unused-vars
  ...
  const checkInstall = () => {
    ...
    if (state.customCrit5e.version < scriptVersion) {
      ...
      switch (state.customCrit5e.version) {
        ...
        case '0.2.1': { // Remove unused settings
          delete state.customCrit5e.logWho;
          delete state.customCrit5e.scriptName;
          break;
        }
      }
      state.customCrit5e.version = scriptVersion;
  ...
  const clog = (message) => {
    if (settings.logMode === 'chat') sendChat(scriptName, message, null, { noarchive: true });
          ...
          case 'changelog':
            ...
            if (changeLog && args[0]) {
              if (args[0] < changeLog.length && args[0] % 3 === 2) {
                sendChat(scriptName, changeLog[args[0]], null, { noarchive: true })
              } else {
 ...
 [{
  ...
  const makeTable = (title, content, columns, substyle = 'std') => { // columns in form of [numColumns, %width ... %width]
    ...
    let header = `<table class="${substyle} ${scriptName}" style="${style[substyle]}"><caption style="${style[Caption]}">${title}</caption>`
    let body = [];
    let footer = `</table>`;
    content.forEach(row => {
      let rowHtml = `<tr>`;
  ...
  const createChangeLog = () => {
    if (changeLog.length > 2) {
      ...
      sendChat(scriptName, `/w gm ${makeTable(`NPC's modified`, mainTableBody, [2, 10, 90], 'log')}`, null, { noarchive: true })
    } else {
      sendChat(scriptName, `/w gm ERROR: Invalid change log contents, can't create table!`, null, { noarchive: true });
    }
  }
